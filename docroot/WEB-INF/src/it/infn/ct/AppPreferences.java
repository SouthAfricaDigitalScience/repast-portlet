    ...
    public InfrastructureInfo[] getEnabledInfrastructures() {
        ...
        for(int i=0,h=0; i<getNumInfrastructures(); i++) {
            ...
            if(appInfrastructuresInfo.get(i).getEnableInfrastructure().equalsIgnoreCase("yes")) {
                ...
                if(getAcronymInfrastructure(i).equalsIgnoreCase("unicore")) {
                ...
                else if(getAcronymInfrastructure(i).equalsIgnoreCase("wsgram")) {
                    // Multi-infrastructure support (field mapping)
                    ...
                    if(_log !=null)
                        _log.info(LS+"GARUDA infrastructure: "
                ...
                else if(getAcronymInfrastructure(i).equalsIgnoreCase("bes-genesis2")) {
                    // Multi-infrastructure support (field mapping)
                    // UNICORE infrastructure requires to specify:
                    // * Keystore file -> bdiiHost first  field of the given ';' separated string
                    // * Password      -> bdiiHost second field of the given ';' separated string
                    // * JSAGA adaptor -> wmsHosts
                    String geneisItems[]  = getBdiiHost(i).split(";");
                    int    numGenesisItems = geneisItems.length;
                    String keyStore = "";
                    String password = "";
                    if(numGenesisItems>0) keyStore=geneisItems[0].trim();
                    if(numGenesisItems>1) password=geneisItems[1].trim();
                    String wmsGenesis[] = { "bes-genesis2://xcg-server1.uvacse.virginia.edu:20443/axis/services/GeniiBESPortType?genii-container-id=93B641B7-9422-EA4C-A90B-CA6A9D98E344" };
                    ...
                    infrastructuresInfo[h++] = new InfrastructureInfo( getAcronymInfrastructure(i)
                                                                      ...
                                                                      ,wmsGenesis);
                    if(_log !=null)
                        _log.info("bes-genesis2 infrastructure: "
                               +LS+" keyStore:      '"+ keyStore    +"'"
                               +LS+" password:      '"+ password    +"'"
                               +LS+" JSAGA adaptor: '"+ wmsHostList +"'"
                                 );
                ...
                else if(getAcronymInfrastructure(i).equalsIgnoreCase("gos")) {
                    // Multi-infrastructure support (field mapping)   
                    // GOS infrastructure requires to specify:   
                    // * JSAGA adaptor -> wmsHosts
                    ...
                    infrastructuresInfo[h++] = new InfrastructureInfo( getAcronymInfrastructure(i)
                                                                      ...
                                                                      ,wmsHostList);
                    if(_log !=null)
                        _log.info("GOS infrastructure: "
                               +LS+" JSAGA adaptor: '" + wmsHostList +"'"
                                 );
                }
                else if(getAcronymInfrastructure(i).equalsIgnoreCase("ssh")) {
                    // Multi-infrastructure support (field mapping)
